...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.address = @"prgd6brc2f.execute-api.us-west-2.amazonaws.com";
        self.port = 443;
        self.path = @"/production/receiver";
    }
    return self;
}
...
- (void)read {
    ...
    if (position) {
        if ([position.email isEqualToString:[self.userDefaults stringForKey:@"email_preference"]]) {
            [self send:position];
        } else {
...
- (void)send:(TCPosition *)position {
    NSURL *request = [TCProtocolFormatter formatPostion:position address:self.address port:self.port path:self.path];
